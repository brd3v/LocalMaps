...
<Input
label="Nome:" name='name' value={formValues.name} onChange={setFromValues}
...
<Input
label="Descrição:" name='description' value={formValues.description} onChange={setFromValues}
...
<Input
label="Contato:" name='contact' value={formValues.contact} onChange={setFromValues}
...
<MapContainer center={{
    lat:coords[0],
    lng:coords[1],
}as LatLngExpression}  zoom={13}
whenCreated={(map) =>{
    map.addEventListener('click', (event:LeafletMouseEvent)=>{
        setFromValues(prev => ({ ...prev, coords: [event.latlng.lat, event.latlng.lng],}))
    })
}}
>
<TileLayer
...
<Section>
    ...
    <CategoryContainer>
        {categories.map(category => (
            ...
            key={category.key}
            onClick={() =>{
                setFromValues(prev => ({...prev, ProgressEvent, category: category.key}))
